...
class BrowseVerticalListAdapter(private val context: Context, private val list: ArrayList<HomePageItem>) : RecyclerView.Adapter<RecyclerView.ViewHolder>() {
    ...
    private fun gridLayoutManager(count: Int, orientation: Int): androidx.recyclerview.widget.GridLayoutManager = androidx.recyclerview.widget.GridLayoutManager(context, count, orientation, false);

    private fun linearLayoutManager(orientation: Int): androidx.recyclerview.widget.LinearLayoutManager = androidx.recyclerview.widget.LinearLayoutManager(context, orientation, false);
    ...
    private fun getLayoutManager(position: Int): RecyclerView.LayoutManager {
        ...
        return when (type) {
            HomePageItem.ALBUM -> {
                ...
                //gridLayoutManager(if(Constants.OFFLINE_ACCESS) { if (albumsSize < 2) albumsSize else 2 } else {2}, androidx.recyclerview.widget.GridLayoutManager.HORIZONTAL);
            }
